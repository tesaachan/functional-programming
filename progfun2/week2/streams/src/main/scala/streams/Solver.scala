...
trait Solver extends GameDef:
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] =
    (for 
      neignbor <- b.legalNeighbors 
    yield (neignbor._1, neignbor._2 :: history)
    ).to(LazyList)
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    if initial.isEmpty then LazyList.empty
      ...
      initial.head #:: from(initial.tail ++ frontPaths, explored ++ frontPaths.map(_._1))
  ...
  lazy val solution: List[Move] = 
    (for
      path <- pathsToGoal
    yield path._2).head.toList.reverse
